      ...
      <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <p className="modal-text">
            Dados de afinidade, posologia e metabolismo extraídos da planilha de referência do usuário,
            com padrão de evidência priorizando dados in vivo/humanos quando disponíveis. A nota sobre
            ligação à albumina do valproato foi pesquisada separadamente (não constava na planilha original).
          </p>
...
function useComposeAnalysis(selected) {
  return useMemo(() => {
    ...
    // interações metabólicas por via comum
    const metabolic = [];
        ...
        <div className="pad">
          {drugs.length < 2 ? (
            ...
            <>
              ...
              {metabolic.map((m, i) => (
                <div className="alert-card alert-red" key={"met" + i}>
                  <AlertTriangle size={16} />
                  <div>
                    <div className="alert-title">{m.a.nome} + {m.b.nome} — via metabólica comum</div>
                    <div className="alert-text">
                      Ambos metabolizados por {m.shared.join(", ")} → pode haver interação por via comum
                      (competição / inibição). Monitorar resposta clínica e considerar ajuste de dose.
                    </div>
                  </div>
                </div>
              ))}
              ...
              {receptorAlerts.map((a) => (
                <div className="alert-card alert-blue" key={a.key}>
                  <AlertTriangle size={16} />
                  <div>
                    <div className="alert-title">Alvo em comum — {a.key}</div>
...
.alert-blue { background: hsla(214,70%,55%,0.07); border-color: hsla(214,70%,55%,0.25); border-left-color: hsl(214 70% 60%); color: hsl(214 70% 70%); }
